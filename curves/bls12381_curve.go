...
import (
	native2 "curveLib-lfzkoala/curves/native"
	bls123812 "curveLib-lfzkoala/curves/native/bls12381"
	"fmt"
	"io"
	"math/big"
...
func (s *ScalarBls12381) Hash(bytes []byte) Scalar {
	...
	xmd := native2.ExpandMsgXmd(native2.EllipticPointHasherSha256(), bytes, dst, 48)
	var t [64]byte
	copy(t[:48], internal.ReverseScalarBytes(xmd))

	return &ScalarBls12381{
		Value: bls123812.Bls12381FqNew().SetBytesWide(&t),
		point: s.point,
	}
}
...
func (s *ScalarBls12381) Zero() Scalar {
	return &ScalarBls12381{
		Value: bls123812.Bls12381FqNew().SetZero(),
		point: s.point,
	}
}
...
func (s *ScalarBls12381) One() Scalar {
	return &ScalarBls12381{
		Value: bls123812.Bls12381FqNew().SetOne(),
		point: s.point,
	}
}
...
func (s *ScalarBls12381) Square() Scalar {
	return &ScalarBls12381{
		Value: bls123812.Bls12381FqNew().Square(s.Value),
		point: s.point,
	}
}

func (s *ScalarBls12381) Double() Scalar {
	v := bls123812.Bls12381FqNew().Double(s.Value)
	return &ScalarBls12381{
		Value: v,
		point: s.point,
	}
...
func (s *ScalarBls12381) Invert() (Scalar, error) {
	value, wasInverted := bls123812.Bls12381FqNew().Invert(s.Value)
	if !wasInverted {
...
func (s *ScalarBls12381) Sqrt() (Scalar, error) {
	value, wasSquare := bls123812.Bls12381FqNew().Sqrt(s.Value)
	if !wasSquare {
...
func (s *ScalarBls12381) Add(rhs Scalar) Scalar {
	...
	if ok {
		return &ScalarBls12381{
			Value: bls123812.Bls12381FqNew().Add(s.Value, r.Value),
			point: s.point,
		}
	} else {
		return nil
...
func (s *ScalarBls12381) Sub(rhs Scalar) Scalar {
	...
	if ok {
		return &ScalarBls12381{
			Value: bls123812.Bls12381FqNew().Sub(s.Value, r.Value),
			point: s.point,
		}
	} else {
		return nil
...
func (s *ScalarBls12381) Mul(rhs Scalar) Scalar {
	...
	if ok {
		return &ScalarBls12381{
			Value: bls123812.Bls12381FqNew().Mul(s.Value, r.Value),
			point: s.point,
		}
	} else {
		return nil
...
func (s *ScalarBls12381) Div(rhs Scalar) Scalar {
	r, ok := rhs.(*ScalarBls12381)
	if ok {
		v, wasInverted := bls123812.Bls12381FqNew().Invert(r.Value)
		if !wasInverted {
			return nil
		}
		v.Mul(v, s.Value)
...
func (s *ScalarBls12381) Neg() Scalar {
	return &ScalarBls12381{
		Value: bls123812.Bls12381FqNew().Neg(s.Value),
		point: s.point,
	}
}
...
func (s *ScalarBls12381) SetBigInt(v *big.Int) (Scalar, error) {
	...
	return &ScalarBls12381{
		Value: bls123812.Bls12381FqNew().SetBigInt(v),
		point: s.point,
	}, nil
}
...
func (s *ScalarBls12381) SetBytes(bytes []byte) (Scalar, error) {
	...
	value, err := bls123812.Bls12381FqNew().SetBytes(&seq)
	if err != nil {
		return nil, err
	}
	return &ScalarBls12381{
...
func (s *ScalarBls12381) SetBytesWide(bytes []byte) (Scalar, error) {
	...
	copy(seq[:], bytes)
	return &ScalarBls12381{
		bls123812.Bls12381FqNew().SetBytesWide(&seq), s.point,
	}, nil
}
...
func (s *ScalarBls12381) Clone() Scalar {
	return &ScalarBls12381{
		Value: bls123812.Bls12381FqNew().Set(s.Value),
		point: s.point,
	}
}
...
func (p *PointBls12381G1) Hash(bytes []byte) Point {
	...
	pt := new(bls123812.G1).Hash(native2.EllipticPointHasherSha256(), bytes, domain)
	return &PointBls12381G1{Value: pt}
}

func (p *PointBls12381G1) Identity() Point {
	return &PointBls12381G1{
		Value: new(bls123812.G1).Identity(),
	}
}

func (p *PointBls12381G1) Generator() Point {
	return &PointBls12381G1{
		Value: new(bls123812.G1).Generator(),
	}
}
...
func (p *PointBls12381G1) Double() Point {
	return &PointBls12381G1{new(bls123812.G1).Double(p.Value)}
}

func (p *PointBls12381G1) Scalar() Scalar {
	return &ScalarBls12381{
		Value: bls123812.Bls12381FqNew(),
		point: new(PointBls12381G1),
	}
}

func (p *PointBls12381G1) Neg() Point {
	return &PointBls12381G1{new(bls123812.G1).Neg(p.Value)}
}
...
func (p *PointBls12381G1) Add(rhs Point) Point {
	...
	if ok {
		return &PointBls12381G1{new(bls123812.G1).Add(p.Value, r.Value)}
	} else {
		return nil
	}
}
...
func (p *PointBls12381G1) Sub(rhs Point) Point {
	...
	r, ok := rhs.(*PointBls12381G1)
	if ok {
		return &PointBls12381G1{new(bls123812.G1).Sub(p.Value, r.Value)}
	} else {
		return nil
	}
}
...
func (p *PointBls12381G1) Mul(rhs Scalar) Point {
	...
	r, ok := rhs.(*ScalarBls12381)
	if ok {
		return &PointBls12381G1{new(bls123812.G1).Mul(p.Value, r.Value)}
	} else {
		return nil
	}
}
...
func (p *PointBls12381G1) Set(x, y *big.Int) (Point, error) {
	value, err := new(bls123812.G1).SetBigInt(x, y)
	if err != nil {
...
func (p *PointBls12381G1) FromAffineCompressed(bytes []byte) (Point, error) {
	var b [bls123812.FieldBytes]byte
	copy(b[:], bytes)
	value, err := new(bls123812.G1).FromCompressed(&b)
	if err != nil {
		return nil, err
	}
	return &PointBls12381G1{value}, nil
...
func (p *PointBls12381G1) FromAffineUncompressed(bytes []byte) (Point, error) {
	var b [96]byte
	copy(b[:], bytes)
	value, err := new(bls123812.G1).FromUncompressed(&b)
	if err != nil {
		return nil, err
	}
	return &PointBls12381G1{value}, nil
...
func (p *PointBls12381G1) SumOfProducts(points []Point, scalars []Scalar) Point {
	nPoints := make([]*bls123812.G1, len(points))
	nScalars := make([]*native2.Field, len(scalars))
	...
	value, err := new(bls123812.G1).SumOfProducts(nPoints, nScalars)
	if err != nil {
		return nil
	}
...
func (p *PointBls12381G2) Hash(bytes []byte) Point {
	...
	pt := new(bls123812.G2).Hash(native2.EllipticPointHasherSha256(), bytes, domain)
	return &PointBls12381G2{Value: pt}
}

func (p *PointBls12381G2) Identity() Point {
	return &PointBls12381G2{
		Value: new(bls123812.G2).Identity(),
	}
}

func (p *PointBls12381G2) Generator() Point {
	return &PointBls12381G2{
		Value: new(bls123812.G2).Generator(),
	}
}
...
func (p *PointBls12381G2) Double() Point {
	return &PointBls12381G2{new(bls123812.G2).Double(p.Value)}
}
...
func (p *PointBls12381G2) Neg() Point {
	return &PointBls12381G2{new(bls123812.G2).Neg(p.Value)}
}
...
func (p *PointBls12381G2) Add(rhs Point) Point {
	...
	if ok {
		return &PointBls12381G2{new(bls123812.G2).Add(p.Value, r.Value)}
	} else {
		return nil
	}
}
...
func (p *PointBls12381G2) Sub(rhs Point) Point {
	...
	r, ok := rhs.(*PointBls12381G2)
	if ok {
		return &PointBls12381G2{new(bls123812.G2).Sub(p.Value, r.Value)}
	} else {
		return nil
	}
}
...
func (p *PointBls12381G2) Mul(rhs Scalar) Point {
	...
	r, ok := rhs.(*ScalarBls12381)
	if ok {
		return &PointBls12381G2{new(bls123812.G2).Mul(p.Value, r.Value)}
	} else {
		return nil
	}
}
...
func (p *PointBls12381G2) Set(x, y *big.Int) (Point, error) {
	value, err := new(bls123812.G2).SetBigInt(x, y)
	if err != nil {
		return nil, fmt.Errorf("invalid coordinates")
	}
...
func (p *PointBls12381G2) FromAffineCompressed(bytes []byte) (Point, error) {
	var b [bls123812.WideFieldBytes]byte
	copy(b[:], bytes)
	value, err := new(bls123812.G2).FromCompressed(&b)
	if err != nil {
		return nil, err
	}
	return &PointBls12381G2{value}, nil
...
func (p *PointBls12381G2) FromAffineUncompressed(bytes []byte) (Point, error) {
	var b [bls123812.DoubleWideFieldBytes]byte
	copy(b[:], bytes)
	value, err := new(bls123812.G2).FromUncompressed(&b)
	if err != nil {
		return nil, err
	}
	return &PointBls12381G2{value}, nil
...
func (p *PointBls12381G2) SumOfProducts(points []Point, scalars []Scalar) Point {
	nPoints := make([]*bls123812.G2, len(points))
	nScalars := make([]*native2.Field, len(scalars))
	for i, pt := range points {
	...
	value, err := new(bls123812.G2).SumOfProducts(nPoints, nScalars)
	if err != nil {
		return nil
	}
...
func (p *PointBls12381G2) X() *big.Int {
	...
	return new(big.Int).SetBytes(x[:bls123812.WideFieldBytes])
}

func (p *PointBls12381G2) Y() *big.Int {
	y := p.Value.ToUncompressed()
	return new(big.Int).SetBytes(y[bls123812.WideFieldBytes:])
}
...
func (s *ScalarBls12381Gt) Random(reader io.Reader) Scalar {
	value, err := new(bls123812.Gt).Random(reader)
	if err != nil {
		return nil
	}
	return &ScalarBls12381Gt{value}
...
func (s *ScalarBls12381Gt) Zero() Scalar {
	return &ScalarBls12381Gt{new(bls123812.Gt)}
}

func (s *ScalarBls12381Gt) One() Scalar {
	return &ScalarBls12381Gt{new(bls123812.Gt).SetOne()}
}
...
func (s *ScalarBls12381Gt) Square() Scalar {
	return &ScalarBls12381Gt{
		new(bls123812.Gt).Square(s.Value),
	}
}

func (s *ScalarBls12381Gt) Double() Scalar {
	return &ScalarBls12381Gt{
		new(bls123812.Gt).Double(s.Value),
	}
}

func (s *ScalarBls12381Gt) Invert() (Scalar, error) {
	value, wasInverted := new(bls123812.Gt).Invert(s.Value)
	if wasInverted != 1 {
...
func (s *ScalarBls12381Gt) Cube() Scalar {
	value := new(bls123812.Gt).Square(s.Value)
...
func (s *ScalarBls12381Gt) Add(rhs Scalar) Scalar {
	...
	if ok {
		return &ScalarBls12381Gt{
			new(bls123812.Gt).Add(s.Value, r.Value),
		}
	} else {
		return nil
	}
...
func (s *ScalarBls12381Gt) Sub(rhs Scalar) Scalar {
	...
	if ok {
		return &ScalarBls12381Gt{
			new(bls123812.Gt).Sub(s.Value, r.Value),
		}
	} else {
		return nil
	}
...
func (s *ScalarBls12381Gt) Neg() Scalar {
	return &ScalarBls12381Gt{
		new(bls123812.Gt).Neg(s.Value),
	}
}

func (s *ScalarBls12381Gt) SetBigInt(v *big.Int) (Scalar, error) {
	var bytes [bls123812.GtFieldBytes]byte
	v.FillBytes(bytes[:])
...
func (s *ScalarBls12381Gt) Point() Point {
	return &PointBls12381G1{Value: new(bls123812.G1).Identity()}
}
...
func (s *ScalarBls12381Gt) SetBytes(bytes []byte) (Scalar, error) {
	var b [bls123812.GtFieldBytes]byte
	copy(b[:], bytes)
	ss, isCanonical := new(bls123812.Gt).SetBytes(&b)
	if isCanonical == 0 {
...
func (s *ScalarBls12381Gt) SetBytesWide(bytes []byte) (Scalar, error) {
	l := len(bytes)
	if l != bls123812.GtFieldBytes*2 {
		return nil, fmt.Errorf("invalid byte sequence")
	}
	var b [bls123812.GtFieldBytes]byte
	copy(b[:], bytes[:bls123812.GtFieldBytes])

	value, isCanonical := new(bls123812.Gt).SetBytes(&b)
	if isCanonical == 0 {
		return nil, fmt.Errorf("invalid byte sequence")
	}
	copy(b[:], bytes[bls123812.GtFieldBytes:])
	value2, isCanonical := new(bls123812.Gt).SetBytes(&b)
	if isCanonical == 0 {
		return nil, fmt.Errorf("invalid byte sequence")
	}
...
func (s *ScalarBls12381Gt) Clone() Scalar {
	return &ScalarBls12381Gt{
		Value: new(bls123812.Gt).Set(s.Value),
	}
}
