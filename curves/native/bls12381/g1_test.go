...
func TestG1Mul(t *testing.T) {
	g := new(G1).Generator()
	a := Bls12381FqNew().SetRaw(&[native2.FieldLimbs]uint64{
		0x2b568297a56da71c,
		0xd8c39ecb0ef375d1,
		0x435c38da67bfbf96,
		0x8088a05026b659b2,
	})
	b := Bls12381FqNew().SetRaw(&[native2.FieldLimbs]uint64{
		0x785fdd9b26ef8b85,
		0xc997f25837695c18,
		0x4c8dbc39e7b756c1,
		0x70d9b6cc6d87df20,
...
func TestG1Hash(t *testing.T) {
	...
	for _, tst := range tests {
		...
		pt.Hash(native2.EllipticPointHasherSha256(), i, dst)
		require.Equal(t, 1, pt.Equal(ept))
	}
}

func TestSerialization(t *testing.T) {
	a := new(G1).Hash(native2.EllipticPointHasherSha256(), []byte("a"), []byte("BLS12381G1_XMD:SHA-256_SSWU_RO_"))
	b := new(G1).Hash(native2.EllipticPointHasherSha256(), []byte("b"), []byte("BLS12381G1_XMD:SHA-256_SSWU_RO_"))
...
func TestSumOfProducts(t *testing.T) {
	...
	lhs := new(G1).Mul(h0, s)
	rhs, _ := new(G1).SumOfProducts([]*G1{h0}, []*native2.Field{s})
	require.Equal(t, 1, lhs.Equal(rhs))
	...
	_, _ = rhs.SumOfProducts([]*G1{u, h0}, []*native2.Field{c, sHat})
	require.Equal(t, 1, uTilde.Equal(rhs))
}
