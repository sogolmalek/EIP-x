...
func (g1 *G1) Random(reader io.Reader) (*G1, error) {
	...
	return g1.Hash(native2.EllipticPointHasherSha256(), seed[:], dst), nil
}

// Hash uses the hasher to map bytes to a valid point
func (g1 *G1) Hash(hash *native2.EllipticPointHasher, msg, dst []byte) *G1 {
	var u []byte
	var u0, u1 fp
	var r0, r1, q0, q1 G1
	...
	switch hash.Type() {
	case native2.XMD:
		u = native2.ExpandMsgXmd(hash, msg, dst, 128)
	case native2.XOF:
		u = native2.ExpandMsgXof(hash, msg, dst, 128)
	}
...
func (g1 *G1) Mul(a *G1, s *native2.Field) *G1 {
	bytes := s.Bytes()
	return g1.multiply(a, &bytes)
}

func (g1 *G1) multiply(a *G1, bytes *[native2.FieldBytes]byte) *G1 {
	var p G1
	precomputed := [16]*G1{}
...
func (g1 *G1) SumOfProducts(points []*G1, scalars []*native2.Field) (*G1, error) {
	const Upper = 256
	const W = 4
