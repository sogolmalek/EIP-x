...
func (g2 *G2) Random(reader io.Reader) (*G2, error) {
	...
	return g2.Hash(native2.EllipticPointHasherSha256(), seed[:], dst), nil
}

// Hash uses the hasher to map bytes to a valid point
func (g2 *G2) Hash(hash *native2.EllipticPointHasher, msg, dst []byte) *G2 {
	var u []byte
	var u0, u1 fp2
	var r0, r1, q0, q1 G2
	...
	switch hash.Type() {
	case native2.XMD:
		u = native2.ExpandMsgXmd(hash, msg, dst, 256)
	case native2.XOF:
		u = native2.ExpandMsgXof(hash, msg, dst, 256)
	}
...
func (g2 *G2) Mul(a *G2, s *native2.Field) *G2 {
	bytes := s.Bytes()
	return g2.multiply(a, &bytes)
}

func (g2 *G2) multiply(a *G2, bytes *[native2.FieldBytes]byte) *G2 {
	var p G2
	precomputed := [16]*G2{}
...
func (g2 *G2) SumOfProducts(points []*G2, scalars []*native2.Field) (*G2, error) {
	const Upper = 256
	const W = 4
