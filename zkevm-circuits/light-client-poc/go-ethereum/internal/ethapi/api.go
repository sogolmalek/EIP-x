...
// GetProof returns the Merkle-proof for a given account and optionally some storage keys.
func (s *BlockChainAPI) GetAccessList(ctx context.Context, blockNumber rpc.BlockNumber) (types.AccessList, error) {
	return nil, nil
}
...
// GetAccessListByNumber returns.
func (s *TransactionAPI) GetAccessListByNumber(ctx context.Context, blockNr rpc.BlockNumber) types.AccessList {
	accessList, err := s.b.AccessListByNumber(ctx, blockNr)
	if err != nil {
		return nil
	}
	return accessList
