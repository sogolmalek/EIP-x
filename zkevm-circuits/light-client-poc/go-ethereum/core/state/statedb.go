...
func (s *StateDB) Commit(deleteEmptyObjects bool) (common.Hash, error) {
	_, root, err := s.CommitWithBlock(deleteEmptyObjects, nil)
	return root, err
}

func (s *StateDB) CommitWithBlock(deleteEmptyObjects bool, blockNo *big.Int) (types.AccessList, common.Hash, error) {
	// Short circuit in case any database failure occurred earlier.
	if s.dbErr != nil {
		return nil, common.Hash{}, fmt.Errorf("commit aborted due to earlier error: %v", s.dbErr)
	}
	...
	accessList := s.CollectAccessLists()
	...
	if blockNo != nil && len(accessList) > 0 {
		sdb, err := New(root, s.db, s.snaps)
		if err != nil {
			return nil, common.Hash{}, err
		}
		if err := sdb.DumpAccountProofs(blockNo, accessList); err != nil {
			return nil, common.Hash{}, err
		}
	}

	return accessList, root, nil
}

func (s *StateDB) CollectAccessLists() types.AccessList {

	collected := make(map[common.Address]map[common.Hash]interface{})

	for addr := range s.stateObjects {
		obj := s.stateObjects[addr]
		collected[obj.address] = make(map[common.Hash]interface{}, 0)
		for key := range obj.dirtyStorage {
			collected[obj.address][key] = nil
		}
	...
	accessList := make([]types.AccessTuple, 0)
	for addr, storage := range collected {
...
func (s *StateDB) DumpAccountProofs(blockNo *big.Int, accessList types.AccessList) error {
	proofs := make([]*AccountProof, 0)
	for _, accessTuple := range accessList {
		proof, err := s.GetProofFull(accessTuple.Address, accessTuple.StorageKeys)
		if err != nil {
			return err
		}
	...
	if encoded, err := json.Marshal(proofs); err == nil {
		fileName := fmt.Sprintf("access-list-%d.json", blockNo.Uint64())
...
func (n *TrieProofList) Put(key []byte, value []byte) error {
	*n = append(*n, hexutil.Encode(value))
	return nil
}

func (n *TrieProofList) Delete(key []byte) error {
	panic("not supported")
}

func (s *StateDB) GetProofFull(address common.Address, storageKeys []common.Hash) (*AccountProof, error) {
	var (
		keyLengths   = make([]int, len(storageKeys))
		storageProof = make([]StorageProof, len(storageKeys))
		storageTrie  Trie
	...
	if storageTrie, err = s.StorageTrie(address); err != nil {
		return nil, err
	}
	...
	for i, key := range storageKeys {
		...
		if storageTrie == nil {
			storageProof[i] = StorageProof{outputKey, &hexutil.Big{}, []string{}}
			continue
		}
		var proof TrieProofList
		if err := storageTrie.Prove(crypto.Keccak256(key.Bytes()), &proof); err != nil {
			return nil, err
		}

		value := (*hexutil.Big)(s.GetState(address, key).Big())
		storageProof[i] = StorageProof{outputKey, value, proof}
	}

	// Create the accountProof.
	accountProof, proofErr := s.GetProof(address)
	if proofErr != nil {
		return nil, proofErr
	}
	...
	return &AccountProof{
		...
		Balance:      (*hexutil.Big)(s.GetBalance(address)),
		CodeHash:     codeHash,
		Nonce:        hexutil.Uint64(s.GetNonce(address)),
