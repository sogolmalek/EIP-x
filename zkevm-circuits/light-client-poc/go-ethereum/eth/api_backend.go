...
func (b *EthAPIBackend) AccessListByNumber(ctx context.Context, number rpc.BlockNumber) (types.AccessList, error) {
	if number < 0 {
		return nil, errors.New("invalid arguments; expect hash and no special block numbers")
	}
	accessList := b.eth.blockchain.GetAccessListByNumber(uint64(number))
